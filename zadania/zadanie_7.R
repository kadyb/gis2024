...
# Obliczenie sumy, średniej i odchylenia standardowego dla rastra
statystyki <- data.frame(
  Suma = global(DEM, "sum",na.rm = TRUE),
  Średnia = global(DEM, "mean",na.rm = TRUE),
  Odchylenie_standardowe = global(DEM, "sd",na.rm = TRUE)
)
