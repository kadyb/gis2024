...
Do obliczenia podstawowych statysyk opsiowych temperatury możemy wykorzystać
funkcję `summary()`.
...
tj. *SpatVector*. W tym celu należy użyć funkcji `vect()` określająć nazwy
...
* walidacja podzbiorem (*train / test split*) -- zbiór danych wejściowych
...
Podziału na podzbiory można dokonać używająć funkcji `sample()` w celu wylosowania
...
```{r}
gst = gstat(formula = TEMP ~ 1, locations = ~X + Y, data = meteo_df)
v = variogram(gst, width = 0.6)
plot(v)
```

```{r warning=FALSE}
fv = fit.variogram(v, vgm(psill = 4, model = "Sph", range = 2, nugget = 2))
fv
plot(v, model = fv)
```

```{r}
mdl = gstat(formula = TEMP ~ 1, locations = ~X + Y, data = meteo_df, model = fv)
...
Dodatkowo wykorzystaj metodę "cieńkiej płytki" (*thin plate spline*) z pakietu
**fields** (funkcja `Tps()`).
